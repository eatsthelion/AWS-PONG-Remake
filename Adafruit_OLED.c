...
void writeCommand(unsigned char c) {
    ...
    unsigned long dummy;
    //write pin 21 (OLED_DC) LOW
    GPIOPinWrite(GPIOA1_BASE,0x2,0x00);

    //Enable chip select
    MAP_SPICSEnable(GSPI_BASE);

    //Sends data byte c to OLED
    MAP_SPIDataPut(GSPI_BASE,c);

    //buffer clear from dummy
...
*/
  ...
  GPIOPinWrite(GPIOA3_BASE, 0x40, 0);	// RESET = RESET_LOW

  for(delay=0; delay<100; delay=delay+1);// delay minimum 100 ns

  GPIOPinWrite(GPIOA3_BASE, 0x40, 0x40);	// RESET = RESET_HIGH
